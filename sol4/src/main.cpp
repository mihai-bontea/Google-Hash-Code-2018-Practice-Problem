		...
		for (int j = 1; j <= data.nr_columns; ++j)
			if (!in_a_slice(slices, make_pair(i, j), data.max_size))
			{
				...
				for (auto it = all_rect_config.begin(); it != all_rect_config.end(); ++it)
				{
					...
					double ratio = max(ingr_in_area.first, ingr_in_area.second) / min(ingr_in_area.first, ingr_in_area.second);

					// Compute a score for the slice
					int size = ((i2 - i + 1) * (j2 - j + 1));
					double score = size - (ingr_wasted * ratio * 10);

					new_slice = Slice(i, j, i2, j2, score);

					// Make sure the new slice doesn't overlap with previous
					if (!slice_overlaps(slices, new_slice, data.max_size))
						if (new_slice.score > best_slice.score)
...
int main()
{
	...
	const string out_prefix = "../../output_files/sol4/";
